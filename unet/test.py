...
def show_image(image_1, image_2, image_3, i ):
    ...
    output_dir = os.path.join("/root/notebooks/unet/outputs/unet_cat", f'{i}_compare.png')
...
if __name__ == "__main__":
    ...
    #model = RetinexModel().to(device)
    ssim = SSIM(data_range=1.0, size_average=True, channel=3)
    psnr =  PeakSignalNoiseRatio().to(device)
    criterion = torch.nn.MSELoss()

    try:
        model.load_state_dict(torch.load('/root/notebooks/unet/train_weight/weight_unet_tdn_2.ph'))
