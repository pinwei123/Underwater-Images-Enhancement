...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.cga_down1 = CGAFusion(dim=128)
        self.cga_down2 = CGAFusion(dim=256)
        self.cga_down3 = CGAFusion(dim=512)
        self.cga_up1 = CGAFusion(dim=256)
        self.cga_up2 = CGAFusion(dim=128)
        self.cga_up3 = CGAFusion(dim=64)
    ...
    def forward(self, x):
        ...
        x2 = self.cga_down1(x2_t, x2)
        x3 = self.down2(x2, z)
        x3_t = self.down2_t(x2)
        x3 = self.cga_down2(x3_t, x3)
        x4 = self.down3(x3, x3)   #沒要做
        x4_t = self.down3_t(x3)
        x4 = self.cga_down3(x4_t, x4)
