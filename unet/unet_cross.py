...
class Downsample(nn.Module):
    def __init__(self, n_feat, heads):
        super(Downsample, self).__init__()

        self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat//2, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelUnshuffle(2),
                                  TransformerBlock(dim=n_feat*2, num_heads=heads, ffn_expansion_factor=2.0, bias=True, LayerNorm_type='WithBias'))
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        k = self.key_proj(key)      # (B, C, H, W)
        v = self.value_proj(value)  # (B, C, H, W)  
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down1_t = Downsample(n_feat=64, heads=1)
        self.down2_t = Downsample(n_feat=128, heads=2)
        self.down3_t = Downsample(n_feat=256, heads=4)
        self.up1_t = Upsample(n_feat=512, heads=4)
        self.up2_t = Upsample(n_feat=256, heads=2)
        self.up3_t = Upsample(n_feat=128, heads=1)
        self.cross_down1 = CrossAttention(dim=128, num_heads=1)
        self.cross_down2 = CrossAttention(dim=256, num_heads=2)
        self.cross_down3 = CrossAttention(dim=512, num_heads=4)
        self.cross_up1 = CrossAttention(dim=256, num_heads=4)
        self.cross_up2 = CrossAttention(dim=128, num_heads=2)
        self.cross_up3 = CrossAttention(dim=64, num_heads=1)
    ...
    def forward(self, x):
        ...
        x2 = self.cross_down1(x2, x2_t, x2_t)
        x3 = self.down2(x2, z)
        x3_t = self.down2_t(x2)
        x3 = self.cross_down2(x3, x3_t, x3_t)
        x4 = self.down3(x3, x3)   #沒要做
        x4_t = self.down3_t(x3)
        x4 = self.cross_down3(x4, x4_t, x4_t)
        ...
        x5_t = self.up1_t(x4)
        x5 = self.cross_up1(x5, x5_t, x5_t)
        ...
        x6_t = self.up2_t(x5)
        x6 = self.cross_up2(x6, x6_t, x6_t)
