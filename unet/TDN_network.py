...
class DecomNet(nn.Module):
    def __init__(self, inp_channel = 3, channel=32, kernel_size=3):
...
class Upsample(nn.Module):
    def __init__(self, in_channels):
        ...
        self.body = nn.Sequential(
            nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            nn.BatchNorm2d(in_channels // 2),
            nn.ReLU(inplace=True)
        )
...
class TDN(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        if inp_dec_level2.shape[2:] != out_enc_level2.shape[2:]:
            target_size = (min(inp_dec_level2.shape[2], out_enc_level2.shape[2]),  # 找到較小的 H
                           min(inp_dec_level2.shape[3], out_enc_level2.shape[3]))  # 找到較小的 W
            
            # 確保兩個張量的 H, W 一致
            inp_dec_level2 = F.interpolate(inp_dec_level2, size=target_size, mode='bilinear', align_corners=False)
            out_enc_level2 = F.interpolate(out_enc_level2, size=target_size, mode='bilinear', align_corners=False)
        ...
        if inp_dec_level1.shape[2:] != out_enc_level1.shape[2:]:
            target_size = (min(inp_dec_level1.shape[2], out_enc_level1.shape[2]),  # 取最小的高度
                           min(inp_dec_level1.shape[3], out_enc_level1.shape[3]))  # 取最小的寬度
            
            # 將兩個張量都縮放到相同大小
            inp_dec_level1 = F.interpolate(inp_dec_level1, size=target_size, mode='bilinear', align_corners=False)
            out_enc_level1 = F.interpolate(out_enc_level1, size=target_size, mode='bilinear', align_corners=False)
